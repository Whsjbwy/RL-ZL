...
| intermediate validation | 20 episodes |
| final validation | 100 episodes |
...
在开始正式实验前，Codex 必须审计当前代码是否为最终测试提供了独立 base seed。若验证和最终测试复用了同一组种子，应先增加独立 `final_test_seed` 或评估命令的 `--base-seed` 参数，并为此增加测试。该修改只能改变数据划分，不能改变环境难度。
...
如果评估脚本尚不支持独立 `--base-seed`，Codex 必须先实现并测试该参数，再运行正式最终测试。
